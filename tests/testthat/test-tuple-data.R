# Coverage for the tuple/complex data & init interop (Phase 2 of the
# complex-tuple interop plan): the pure-R flattener (`R/tuples.R`), its
# wiring into `.newstan_run_service()` / `fit$unconstrain_variables()`, and
# the windowed-complex storage in `src/r_data_context.cpp`. See the plan's
# Part B for the normative var_context contract and Part C for the canonical
# R shapes exercised below.
...
test_that("flattening reproduces the Part B worked example exactly", {
  # array[2] tuple(complex_vector[3], real) acv
  declared <- list(acv = list(
...
test_that("array[2,3] tuple(int, real) flattens in blocked-AoS order", {
  ...
  # Hand-computed blocked-AoS order (A3): enclosing-array elements enumerated
...
test_that("real-valued input is accepted where a tuple slot declares complex", {
  declared <- list(tad = list(
    type = data.frame(type = c("int", "complex"), dimensions = c(0, 0)),
    dimensions = 1
  ))
  out <- .newstan_flatten_tuple_values(
    list(tad = list(list(1L, 2), list(3L, 4))), declared
  )
...
test_that("a shape mismatch across enclosing array elements errors", {
  ...
  expect_error(
    .newstan_flatten_tuple_values(
      list(g = list(list(c(1, 2)), list(c(1, 2, 3)))), declared
    ),
...
test_that("a named tuple list errors", {
  ...
  expect_error(
    .newstan_flatten_tuple_values(
      list(td = list(a = 1.5, b = c(2, 3))), declared
    ),
    "unnamed list"
  )
})
...
test_that("a list for a variable not declared as a tuple errors", {
  ...
  # Also covers a missing declaration entirely (same error class per Part D).
  expect_error(
...
test_that("a dotted-name collision with a generated leaf errors", {
  ...
  expect_error(
    .newstan_flatten_tuple_values(
      list(td = list(1.5, c(2, 3)), `td.1` = 99), declared
    ),
...
test_that("wrong tuple arity errors", {
  ...
  expect_error(
    .newstan_flatten_tuple_values(
      list(td = list(1.5, c(2, 3), 99)), declared
    ),
    "length 2"
  )
})
...
#
# Pins the A2-A4 stanc var_context reading contracts against upgrades. If
# this section fails after a stanc upgrade, suspect A3 (blocked-AoS tuple
# array flattening) or A4 (windowed vals_c for tuple-slot complex leaves)
# codegen drift first, not a Stan-library API change.

.newstan_battery_data <- function() {
  list(
    zd = 1 + 2i,
    zv = c(1 + 1i, 2 - 2i),
...
# constrained-parameter form (A2/A5) -- never the bracketed form -- so the
# actual bracket strings used to index the draws always come from calling
  ...
  for (i in 1:2) {
    for (j in 1:2) {
      out <- c(
        out,
        stats::setNames(
          data$t2d[[i]][[j]][[1]], paste0("t2d_out.", i, ".", j, ":1")
        ),
        stats::setNames(
          data$t2d[[i]][[j]][[2]], paste0("t2d_out.", i, ".", j, ":2")
        )
      )
    }
  }
...
test_that("the battery model echoes every input exactly via generated quantities", {
  ...
  # The expected keys are native dotted/colon names (A2/A5); the actual
  # lookup keys come from calling the real `.newstan_bracket_names()` on
  # them, not from a hand-typed bracket string.
...
test_that("unconstrain_variables() is the identity for unbounded tuple/complex parameters", {
  ...
  upars <- fit$unconstrain_variables(list(t = list(0.5, c(1, 2)), z = 0.1 + 0.2i))
