...
and every service result includes its normalized configuration arguments
(large execution inputs are omitted).
...
diagnostics, a Stan return code, and the normalized configuration argument list.
...
`sampling()` accepts precomputed inverse metric values through `inv_metric`.
This corresponds to Stan's `metric_file` input.
